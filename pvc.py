...
class FileProcessor:
    def __init__(self, filename, block_size=D_BLOCK_SIZE, n_blocks=D_N_BLOCKS,
            length_mult=D_LENGTH_MULT, pitch_mult=D_PITCH_MULT,
            f_pitch_mult=D_F_PITCH_MULT, f_filter_size=D_F_FILTER_SIZE, f_corr=D_F_CORR):
        ...
        self.pvc = [PhaseVocoder(self.rate, self.block_size) for i in range(self.in_file.channels)]

        self.indices = np.arange(self.pvc[0].fft_size)
    
    def run(self):
        t = 0
        for block in self.in_file.blocks(blocksize=self.block_size, overlap=(self.block_size-self.in_shift), always_2d=True):
    ...
    def process_block(self, block, channel):
        magnitude, phase, frequency = self.pvc[channel].analyze(block, self.in_shift)
                
        contour = None
        if self.f_corr and (self.pitch_mult != 1 or self.f_pitch_mult != 1):
            contour = np.maximum(scipy.ndimage.maximum_filter1d(magnitude,self.f_filter_size),0.001)
            ...
            if self.f_pitch_mult != 1:
                contour = np.interp(self.indices/self.f_pitch_mult,self.indices,contour,0,0)
        ...
        if self.pitch_mult != 1:                
            ...
            frequency = np.interp(self.indices/self.pitch_mult,self.indices,frequency,0,0)*self.pitch_mult
        ...
        out_block = self.pvc[channel].synthesize(magnitude, frequency, self.out_shift)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b","--block_size", type=int, default=D_BLOCK_SIZE, help="the block size to use when processing")
    parser.add_argument("-n","--n_blocks", type=int, default=D_N_BLOCKS, help="the number of overlapped windows per block")
    parser.add_argument("-l","--length_mult", type=float, default=D_LENGTH_MULT, help="the factor to scale the length of the input file by")
    parser.add_argument("-p","--pitch_mult", type=float, default=D_PITCH_MULT, help="the factor to scale the pitch of the input file by")
    parser.add_argument("-F","--formant_corr", help="perform formant correction on the pitch shifted audio", action="store_true")
    parser.add_argument("-f","--formant_mult", type=float, default=D_F_PITCH_MULT, help="the factor to scale the pitch of the formants of the input file by (implies -F when != 1)")
    parser.add_argument("-g","--formant_filter", type=float, default=D_F_FILTER_SIZE, help="the size of the filter window to use when identifying formants")
    
    args = parser.parse_args()

    processor = FileProcessor(args.in_file, args.block_size, args.n_blocks,
        args.length_mult, args.pitch_mult, args.formant_mult, args.formant_filter, args.formant_corr)
    processor.run()
